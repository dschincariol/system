...
- Shadow-only prediction-market macro expectations: disabled-by-default Kalshi and CME FedWatch sources, normalized read-only storage, PIT-safe `prediction_market_macro_v1.*` feature ids, and replay support without live trading authority.
- Shadow-only Polymarket event signals: disabled-by-default public Gamma/Data/CLOB ingestion, explicit asset and semantic event mapping, condition/token metadata, PIT-safe `prediction_market_event_v1.*` features, cross-provider dispersion only for explicitly mapped comparable events, and no wallet or trading authority.
- Shadow-only ForecastEx regulated event-contract ingestion: disabled-by-default daily/intraday CSV polling, optional read-only allowlisted IBKR event-contract market data, provider contract/product/resolution metadata, stale sparse-contract unavailable states, and regulated `prediction_market_event_v1.*` probability ids without order authority.
- Shadow-only Deribit public crypto derivatives ingestion: disabled-by-default BTC/ETH/SOL options, futures, and perpetuals market data, PIT-safe `deribit_crypto_derivatives_v1.*` volatility/positioning features, crypto-only mapping by default, provider readiness diagnostics, and no authenticated trading authority.
- Research-only sportsbook/Betfair-style odds scaffold: disabled-by-default read-only odds feed or historical-file ingestion, no-vig probability normalization, strict narrow asset/research-label mappings, PIT-safe `sports_odds_sector_v1.*` shadow features, event-study tooling, and no broad-market or betting execution authority.
- Sportsbook odds promotion hardening: active/watch plus explicit model-config universe inventory, approved mapping metadata, provider-readiness diagnostics, `sportsbook_odds_promotion_evidence`, and promotion-guard enforcement keep `sports_odds_sector_v1.*` NO-GO unless OOS, net-after-cost, PIT, deconfounding, readiness, and approval checks all pass.
