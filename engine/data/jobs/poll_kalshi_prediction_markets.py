"""Read-only Kalshi macro prediction-market ingestion job."""
...
from engine.data.prediction_market_providers import fetch_kalshi_macro_batch
...
POLL_SECONDS = float(os.environ.get("KALSHI_PREDICTION_MARKET_POLL_SECONDS", "900.0"))
LOG_LEVEL = os.environ.get("LOG_LEVEL", "INFO").upper()

logging.basicConfig(level=getattr(logging, LOG_LEVEL, logging.INFO), format="%(asctime)s %(levelname)s [poll_kalshi_prediction_markets] %(message)s")
...
def _run_once() -> dict:
    ...
    batch = fetch_kalshi_macro_batch(settings=settings, now_ms=started_ms)
    counts: dict[str, int] = {}
    ...
    manager.record_job_status(
        JOB_NAME,
        ok=True,
        message="kalshi prediction-market macro cycle complete",
        meta={"counts": counts, "source_key": SOURCE_KEY, "shadow_only": True},
    )
    put_job_heartbeat(JOB_NAME, OWNER, PID, extra_json=json.dumps(status, separators=(",", ":"), sort_keys=True))
    LOG.info("kalshi prediction-market cycle counts=%s", counts)
    return {"ok": True, "counts": counts}


def main() -> None:
    if os.environ.get("ENGINE_SUPERVISED") != "1":
        print("poll_kalshi_prediction_markets must be launched by supervisor")
        raise SystemExit(1)

    init_db()
    manager = get_manager()
    if not manager.is_job_enabled(JOB_NAME, default=False):
        manager.record_job_status(JOB_NAME, ok=True, message="kalshi prediction-market source disabled")
        raise SystemExit(0)
    ...
    try:
        while True:
            ...
            try:
                ...
                LOG.exception("kalshi_prediction_market_cycle_failed")
                status = record_pipeline_status(
                    JOB_NAME,
                    ok=False,
                    raw_rows=0,
                ...
                manager.record_job_status(JOB_NAME, ok=False, message="kalshi prediction-market cycle failed", error=str(exc), meta={"source_key": SOURCE_KEY})
                put_job_heartbeat(JOB_NAME, OWNER, PID, extra_json=json.dumps(status, separators=(",", ":"), sort_keys=True))
