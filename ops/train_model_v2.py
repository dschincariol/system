...
Operational helper script for `train_model_v2`.
"""
...
LOG = logging.getLogger("ops.train_model_v2")
...
def _training_scope():
    ...
    for sym, horizon_s in rows:
        ...
        try:
            ...
            logging.warning("train_model_v2 horizon_parse_failed value=%r err=%s", horizon_s, e)
            continue
...
def _load_oos_return_series(*, symbols, horizons, lookback_days: int, holdout_fraction: float = 0.20, holding_horizon_bars: int = 1) -> dict:
    ...
    try:
        rows = con.execute(
            """
            SELECT e.ts_ms, l.symbol, l.horizon_s, l.impact_z
            FROM labels l
