...
LOG = get_logger("engine.data.jobs.backfill_features")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        log_failure(
            LOG,
            event="backfill_features_failed",
            code="BACKFILL_FEATURES_FAILED",
            message=str(exc),
            error=exc,
            level=logging.ERROR,
            component="engine.data.jobs.backfill_features",
            extra={
                "start_ts_ms": int(args.start_ts_ms),
                "end_ts_ms": int(args.end_ts_ms),
                "symbols": str(args.symbols or ""),
                "feature_ids": str(args.feature_ids or ""),
            },
            persist=False,
        )
        return 1
