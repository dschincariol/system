...
Five prompts, each designed to be handed to a fresh AI coding session (Claude Code or otherwise) to implement a single quick win end-to-end. Each prompt includes context, file locations, function contracts, env vars, SQL schema, and acceptance criteria.

**Recommended execution order:** Prompt 1 → Prompt 2 → Prompt 4 → Prompt 3 → Prompt 5.

Repo root: `C:\Users\dschi\Documents\GitHub\Trading-System-\`

---

## Prompt 1 — Harvey/Liu/Zhu t > 3.0 Promotion Gate

**Context.** This is a full-stack Python supervised trading system (~400+ files, SQLite WAL) with a champion/challenger ML loop. The current promotion logic in `engine/strategy/champion_manager.py` promotes challengers when their marketplace score exceeds the champion by a margin. It does **not** correct for multiple-hypothesis testing. Per Harvey/Liu/Zhu (2016), ~95% of published factors are false positives at the standard t > 2.0 threshold. Industry-standard is t > 3.0 plus Deflated Sharpe Ratio (Bailey & de Prado 2014) and Benjamini-Hochberg FDR control.
...
**SQLite schema additions (via `engine/runtime/storage.py` migration pattern).**
```sql
...
## Prompt 2 — Combinatorial Purged Cross-Validation (de Prado)

**Context.** Same system as Prompt 1. Current backtesting leaks across the time axis because it uses naive splits. Marcos López de Prado's *Advances in Financial Machine Learning* (2018) introduced Combinatorial Purged Cross-Validation (CPCV) and the Probability of Backtest Overfitting (PBO) metric. CPCV is the gold standard for robust time-series backtesting — it generates many non-overlapping backtest paths, purges label overlap between train/test, and embargoes post-test bars.
...
**Constraints.**
- Respect existing SQLite storage patterns.
...
## Prompt 3 — tsfresh Automated Feature Extraction

**Context.** Same system. Features today are hand-coded in `engine/strategy/feature_registry.py` across 10 groups. WorldQuant and quantitative funds generate millions of candidate features automatically. `tsfresh` (Python package) computes 700+ time-series features (FFTs, entropy, autocorrelations, peak counts, etc.) from a single window of prices. Feeding these into the existing schema-driven feature pipeline gives the champion/challenger loop a much larger hypothesis space to discover alpha.
...
- `engine/strategy/tsfresh_features.py`:
  - `build_tsfresh_window(symbol: str, end_ts: int, window_s: int) -> pd.DataFrame` — pulls prices from SQLite
...
- Train/serve parity: train an `embed_regressor` with `USE_TSFRESH_FEATURES=1`; assert the model's persisted feature schema includes `tsfresh.*` ids and that live serving uses the same ids.
...
## Prompt 4 — LightGBM Model Family

**Context.** Same system. Current model families (`regime_stats_v2`, `embed_regressor`, `temporal_predictor`) use Bayesian stats, Ridge/MLP, and sequence models. LightGBM is the industry-standard gradient boosted decision tree library — fast, handles non-linearities, robust to feature scaling, state of the art on tabular problems. Adding it as a new family gives the champion/challenger loop a meaningfully different model to compete.

**Goal.** Implement `gbm_regressor` as a new model family that mirrors the `embed_regressor.py` pattern exactly (train job, serve adapter, champion routing, registry persistence).

**Files to create.**
- `engine/strategy/gbm_regressor.py`:
  - `_LGB_MAGIC = b"LGB1"` for blob serialization (matches the pattern in `embed_regressor.py`)
...
- Integration: train `gbm_regressor` on the same labeled dataset used by `embed_regressor`; assert a row lands in `gbm_models` and registry metrics are updated.
...
- Champion/challenger: run the model marketplace job with both `embed_regressor` and `gbm_regressor` registered; assert both appear in the scoring table.

**Constraints.**
- Must follow the existing `embed_regressor.py` pattern exactly for blob serialization, schema persistence, and predictor routing. No novel abstractions.
...
## Prompt 5 — Stacked Ensemble Blending

**Context.** Same system. Today, champion/challenger selects a single winning model and uses its prediction directly. Best-in-class systems (Two Sigma, RenTec) blend many models via stacking — a meta-learner weights the family outputs. This is more robust to regime changes and model-specific failure modes. The existing champion/challenger loop should continue to exist (to retire bad models) but the *live prediction* should be a weighted blend.
...
- SQLite is the center of gravity. Use existing `engine/runtime/storage.py` patterns for connections, migrations, and writes — do not introduce new DB plumbing.
