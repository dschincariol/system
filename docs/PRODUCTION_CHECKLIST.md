...
- Leave `TRADING_REFETCHABLE_PG_DURABILITY_TIER=default` unless a reviewed ingestion soak explicitly accepts relaxed durability for refetchable price/telemetry rows. The only non-default value is `relaxed`, which issues `SET LOCAL synchronous_commit = off` only for hardcoded ingestion price/telemetry write scopes. It must not be used as evidence that order, ledger, broker, audit, event-log, model, trade-outcome, or capital-state writes can relax durability; those write paths stay on default/synchronous Postgres durability in production code.
...
- Tune and verify the ZFS pool before accepting the T1.3c Docker data-root relocation. Run `sudo bash /opt/trading/ops/server/zfs_tuning.sh apply` and `bash /opt/trading/ops/server/zfs_tuning.sh verify`; the verifier must report actual `zdb` ashift `12`, `autotrim=on`, pool-wide `atime=off`, pool-wide `compression=lz4`, and the dedicated PGDATA dataset properties from [DISK_RETENTION_RUNBOOK.md](DISK_RETENTION_RUNBOOK.md). If ashift is wrong, plan a pool migration; do not destroy/recreate the pool in place.
