...
- For the compose operator sidecar, set `OPERATOR_API_TOKEN_FILE` or `OPERATOR_API_TOKEN_SECRET`; live smoke must send the resolved token out-of-band as `OPERATOR_API_TOKEN` or `PIPELINE_SMOKE_OPERATOR_TOKEN` when running tools from an operator shell. Loopback alone does not authorize sidecar reads, mutations, or WebSocket upgrades. Only `/api/operator/ping` is unauthenticated. The dashboard bridge forwards `X-Operator-Token` only from server-side config after dashboard auth succeeds; proxied browser telemetry must fetch a dashboard-authenticated `/operator/ws_ticket` and present that short-lived ticket through `Sec-WebSocket-Protocol` with an allow-listed dashboard/operator `Origin`.
- Keep compose host port publishes loopback-only by default. `TIMESCALE_DANGEROUS_PUBLIC_BIND_HOST`, `REDIS_DANGEROUS_PUBLIC_BIND_HOST`, `MINIO_DANGEROUS_PUBLIC_BIND_HOST`, `MINIO_CONSOLE_DANGEROUS_PUBLIC_BIND_HOST`, and `DASHBOARD_DANGEROUS_PUBLIC_BIND_HOST` default to `127.0.0.1`; the operator sidecar has no host `ports:` mapping by default and is reachable only inside the Compose network on `operator:4001`. The preferred LAN contract exposes only the authenticated dashboard on `:8000`; operator workflows use the same-origin `/operator/` dashboard bridge. Do not expose Timescale, Redis, MinIO, dashboard, or operator directly to a LAN or public interface unless a reviewed firewall/VPN/reverse-proxy design exists.
...
- Leave `OPERATOR_SIDECAR_INTERNAL_ONLY=1`, `OPERATOR_PUBLIC_PORT=` and `OPERATOR_ALLOW_DANGEROUS_PUBLIC_BIND=0` in production. `OPERATOR_PUBLIC_PORT` or `OPERATOR_ALLOW_DANGEROUS_PUBLIC_BIND=1` under the internal-only contract is a preflight blocker because it implies direct sidecar exposure while compose intentionally publishes no host `:4001` port.
- Provide a credential-encryption root with `DATA_SOURCE_MASTER_KEY` or `DATA_SOURCE_MASTER_KEY_FILE`. Production/live preflight accepts only canonical base64 text for exactly 32 random bytes; raw text, placeholders, short strings, low-entropy values, empty files, and ordinary files with group/other permissions fail closed. Prefer the file form:
  ...
  ```
- Provision the backup evidence signing key as an approved secret source before any production/live preflight. Systemd hosts should install `/etc/credstore.encrypted/backup_evidence_hmac_key.cred` with `ops/server/credstore/install.sh` and set `BACKUP_EVIDENCE_HMAC_KEY_SECRET=backup_evidence_hmac_key`; the engine, operator, and preflight units load it with `LoadCredentialEncrypted=backup_evidence_hmac_key:...` and blank `BACKUP_EVIDENCE_HMAC_KEY_FILE` so the runtime does not consume provisioning plaintext. Compose hosts should point `BACKUP_EVIDENCE_HMAC_KEY_FILE` at the Docker secret source file. A direct file source must be readable by the runtime service and mode `0600`; for the `trading` service user, use `trading:trading 0600` or use systemd credentials instead. `root:trading 0640` backup-service files are not valid strict runtime sources.

### GO-R11 Host Hardening

1. Replace inline production secrets in `.env`, `/etc/trading/trading.env`, and service environment sources with approved references before host hardening. At minimum, remove inline `TRADING_MASTER_KEY`, `APP_MASTER_KEY`, and `DASHBOARD_API_TOKEN`; use `TRADING_MASTER_KEY_FILE`, `APP_MASTER_KEY_FILE`, and `DASHBOARD_API_TOKEN_FILE` pointing to non-repo `0600` files, or use `TRADING_MASTER_KEY_SECRET`, `APP_MASTER_KEY_SECRET`, and `DASHBOARD_API_TOKEN_SECRET` with systemd credentials.
2. Apply host hardening from the repository root:
   ```bash
   sudo bash deploy/bin/apply_host_hardening.sh
   ```
   The script delegates swap/zram/ARC setup to `ops/server/memory_pressure_hardening.sh`, installs the engine/operator/backup/restore-drill systemd units with copy-if-changed semantics, enables and starts the long-running supervisor units and backup timers, and enforces `0600 root:root` on the backup-evidence HMAC key file without creating or printing key material. See [MEMORY_PRESSURE_RUNBOOK.md](MEMORY_PRESSURE_RUNBOOK.md) for the memory hardening details.
3. Provision paid feed credentials through approved sources. Configure Polygon with `POLYGON_API_KEY_FILE` or `POLYGON_API_KEY_SECRET`; configure IBKR connection credentials with `IBKR_HOST_FILE`, `IBKR_PORT_FILE`, and `IBKR_CLIENT_ID_FILE` or the matching `*_SECRET` names. File-backed sources must be outside the checkout, readable by the service, non-empty, and mode `0600`.
4. Configure offsite base-backup delivery with either `TS_BASE_BACKUP_OFFSITE_CMD` or `TS_OFFSITE_BACKUP_DEST`. An `s3://` destination is accepted as configured; a local or NAS destination must be an existing writable directory.
5. Re-run the live host and production preflight gates and require zero exits:
   ```bash
   python tools/live_host_readiness_check.py
   python -m engine.runtime.prod_preflight
   ```
   In live mode, `prod_preflight.py` blocks on `[live-host-readiness]` reason codes until swap, systemd unit state, backup key mode, inline-secret absence, paid feed credential source, and offsite backup destination checks all pass.
...
- On host `bart`, keep the CPU power policy boot-enforced through `trading-cpu-power-policy.service`. It selects the `performance` power profile and AMD EPP `performance` for lower scheduling latency and sustained clocks, with the known cost of higher watts, heat, and fan activity. Keep `PREFLIGHT_REQUIRE_CPU_POWER_POLICY=1` on production preflight so post-boot drift fails non-zero, and watch `component_health.cpu_power_policy` from `observability_snapshot` for continuous advisory drift detection. Verify without sudo using `bash ops/server/cpu_power_policy.sh verify`; see [CPU_POWER_POLICY.md](CPU_POWER_POLICY.md) for revert steps and ROCm/GPU thermal composition rules.
- Keep `TRADING_DEPENDENCY_PROFILE=cpu`, `RUNTIME_HARDWARE_PROFILE=cpu`, all `*_DEVICE=cpu`, `NVIDIA_TELEMETRY_ENABLED=0`, `GPU_THROTTLE_ENABLE=0`, `PINNED_ENABLE=0`, `PINNED_PREFETCH=0`, `TORCH_ALLOW_TF32=0`, `CUDNN_ALLOW_TF32=0`, and `CUDNN_BENCHMARK=0` for the default AMD/CPU deployment. NVIDIA CUDA requires both `TRADING_DEPENDENCY_PROFILE=nvidia-cuda` and `RUNTIME_HARDWARE_PROFILE=nvidia`. AMD/ROCm is an explicit opt-in path: use `TRADING_DEPENDENCY_PROFILE=amd-rocm`, `RUNTIME_HARDWARE_PROFILE=amd-rocm`, `TRADING_ACCELERATION_PROFILE=amd-rocm`, and the compose ROCm overlay only on a host that passes [ROCM_ACCELERATION.md](ROCM_ACCELERATION.md) validation. CPU, NVIDIA CUDA, and ROCm full installs are lock-backed and must use `pip install --require-hashes`. `prod_preflight.py` compares the live venv's installed direct CPU runtime packages with `requirements.in` and `requirements.lock.txt`; drift is a live-mode hard blocker, and production safe-mode hosts can enforce the same blocker with `PROD_PREFLIGHT_REQUIRE_DEPENDENCY_LOCK_MATCH=1`. `PROD_PREFLIGHT_SKIP_DEPENDENCY_DRIFT=1` is diagnostic-only and leaves preflight warning/not production-ready. See [DEPENDENCY_PROFILES.md](DEPENDENCY_PROFILES.md) for verification and CPU rollback commands.
...
- Keep `PREFLIGHT_REQUIRE_DOCKER_RUNTIME_EVIDENCE=1` for compose production and refresh `/var/backups/trading/evidence/docker_runtime_inspect.json`, `redis_config_get.txt`, and `postgres_pg_settings.json` after compose changes or restarts. `prod_preflight.py` and `/api/health.effective_runtime_state` must prove actual Docker CPU, memory, memswap, `/dev/shm`, log caps, port bindings, mounts, Redis maxmemory, and effective Postgres settings. Missing effective evidence is not production-ready.
- For the same host, set `INGESTION_TUNING_PROFILE=host_32t_123g` and keep `INGESTION_TUNING_MAX_TOTAL_DB_CONNECTIONS=32`, `INGESTION_TUNING_MAX_BUFFERED_ROWS=1200000`, `INGESTION_CHILD_TS_PG_POOL_SIZE=3`, `INGESTION_CHILD_TIMESCALE_POOL_MAX_SIZE=4`, and `INGESTION_CHILD_TIMESCALE_PRICES_POOL_MAX_SIZE=4`. The profile raises ingestion writer batches and selected parent pools while reducing durable async-writer spool depth so buffered-row risk does not grow, and child feed jobs get smaller explicit pools to avoid multiplying connections across processes. `start_ingestion.py` and `prod_preflight.py` fail closed when explicit queue, spool, or pool overrides exceed hard bounds or the combined DB pool/queue budget. During soak, confirm `/api/health` and `runtime_meta.ingestion_state.writer_diagnostics` show `async_price_persistence.queue_depth` below the high watermark, `async_price_persistence.spool_pending_bytes` below `ASYNC_PRICE_WRITER_SPOOL_MAX_BYTES`, `async_price_persistence.spool_oldest_age_ms` near the normal flush interval rather than growing, `async_price_persistence.spooled_rows` increasing with accepted writes, `async_price_persistence.spool_deleted_rows` increasing after committed database writes, `async_price_persistence.backpressure_active=false`, `async_price_persistence.backpressure_recovered_events` incrementing after any pressure clears, `pg_price_storage.write_circuit_open=false`, `pg_price_storage.backpressure_active=false`, low flush latency, no dead letters, no rejected rows, no dropped rows, `async_price_persistence.residual_loss_rows=0`, `async_price_persistence.spool_corruption_events=0`, `telemetry_append_buffer.queue_depth` and `telemetry_append_buffer.oldest_age_ms` returning to near the normal flush interval, `telemetry_append_buffer.backpressure_active=false`, `telemetry_append_buffer.deleted_rows` increasing after committed non-price telemetry writes, no non-price telemetry drops, `options.poll.durable_buffer.pending_rows` and `pending_bytes` returning to zero or the normal flush interval, `options.poll.durable_buffer.deleted_rows` tracking spooled options rows after commits, `options.poll.durable_buffer.rejected_rows=0`, `options.poll.durable_buffer.dropped_rows=0`, `options.poll.durable_buffer.backpressure_active=0`, no options durable replay/delete/corruption failures, and no Timescale backpressure before live enablement.
- Keep `INGESTION_SOAK_REQUIRE_EVIDENCE=1` or `PREFLIGHT_REQUIRE_INGESTION_SOAK_EVIDENCE=1` for production soak acceptance. `/api/health.ingestion_soak`, `/api/readiness`, and `prod_preflight.py --json` must prove current writer health and applied Timescale policy, not just configured knobs. Treat `ingestion_soak:writer_evidence_missing`, active backpressure, queue/spool pressure, old durable spool rows, nonzero dead-letter/drop/rejection/residual-loss/corruption counters, COPY fallback counts above policy, write/flush failures, open Redis cache circuit, missing hypertables, chunk-interval drift, missing compression jobs, or missing scoring indexes as hard blockers before live enablement. If the runtime cannot query Timescale directly, archive read-only policy evidence in `INGESTION_SOAK_TIMESCALE_POLICY_JSON` or `PREFLIGHT_INGESTION_SOAK_POLICY_JSON`.
...
- Install locked CPU/default Python dependencies with `TRADING_DEPENDENCY_PROFILE=cpu python -m pip install --require-hashes -r requirements.txt`.
...
- Compose production sets `TRADING_REFETCHABLE_PG_DURABILITY_TIER=relaxed` for refetchable price/telemetry ingestion after the reviewed soak profile. Valid values are `default` and `relaxed`; empty/unset uses `default`, and invalid values fail production preflight under `refetchable_pg_durability`. This issues `SET LOCAL synchronous_commit = off` only for hardcoded ingestion price/telemetry write scopes and must show up in Postgres price-sidecar, Timescale, telemetry append-buffer, and production preflight durability snapshots as `tier=relaxed` / `relaxed=true`. It must not be used as evidence that order, ledger, broker, risk, audit, event-log, model, trade-outcome, or capital-state writes can relax durability; those write paths stay on default/synchronous Postgres durability in production code, and protected tables are rejected by the helper if they are passed to a relaxed-durability scope.
- Before calling the ingestion spine GO on a host, run `python tools/ingestion_spine_benchmark.py --postgres-dsn postgresql://127.0.0.1:5432/... --include-values-fallback` against the local Timescale/Postgres service and archive the JSON from `var/artifacts/ingestion_spine_benchmark.json`. The non-price section must report `dropped_rows=0` and `pending_rows_after=0`; the price sections must show `write_path=copy_staging` for the COPY run and `write_path=values_upsert_copy_disabled` for the fallback run. If no local backend is available, run `--skip-postgres` and record that only the durable-spool smoke benchmark was captured.
...
- On a single-server systemd host, confirm `/etc/credstore.encrypted/` contains encrypted credentials named `master_key`, `pg_password_app`, `pg_password_ingest`, `pg_password_reader`, `redis_password`, `object_store_access_key`, `object_store_secret_key`, `dashboard_api_token`, `operator_api_token`, and `backup_evidence_hmac_key` when signed backup evidence is required, and that service units expose only the needed entries with `LoadCredentialEncrypted=`.
...
- Confirm storage placement before first bring-up. Production compose uses explicit bind mounts, not Docker named volumes, for Timescale PGDATA/`pg_wal`, Redis appendonly data, MinIO object data, runtime data/logs, artifact caches, training dataset caches, and backup evidence. Keep `PREFLIGHT_REQUIRE_ZFS_STORAGE=1`, `PREFLIGHT_STORAGE_REQUIRE_VISIBLE_HOST_PATHS=1`, `TRADING_ALLOWED_STORAGE_FS_TYPES=zfs`, `TRADING_ALLOWED_STORAGE_PREFIXES=/zpool,/dbpool,/auxpool,/var/backups/trading`, `TRADING_TIMESCALE_DATA=/dbpool/trading/timescaledb/data`, `TRADING_REDIS_DATA=/auxpool/trading/redis`, `TRADING_MINIO_DATA=/auxpool/trading/minio`, `TRADING_RUNTIME_DATA=/auxpool/trading/runtime/data`, `TRADING_RUNTIME_LOGS=/auxpool/trading/runtime/logs`, and `TRADING_BACKUP_ROOT=/var/backups/trading`. The compose `storage-placement-preflight` service must complete successfully before TimescaleDB starts; production preflight and `/api/health.storage_wal_guards.storage_placement` must show every high-volume target with `evidence_status=satisfied`, `evidence_level=verified_mount`, `filesystem_type=zfs`, and the expected `host_source`, `container_destination`, `mount_source`, `mount_point`, and `mount_options`, not prefix-only evidence.
- Provision and verify the three-pool storage layout before accepting live database writes. Run `sudo bash /opt/trading/ops/server/provision_storage_pools.sh apply --dry-run`, then the gated apply from [DISK_RETENTION_RUNBOOK.md](DISK_RETENTION_RUNBOOK.md), then `bash /opt/trading/ops/server/provision_storage_pools.sh verify`. Verify dbpool's PGDATA tuning with `TRADING_ZFS_POOL=dbpool TRADING_ZFS_DATA_DATASET=dbpool/data TRADING_ZFS_PGDATA_DATASET=dbpool/trading/timescaledb/data bash /opt/trading/ops/server/zfs_tuning.sh verify`. Do not run the DB-oriented `zfs_tuning.sh apply` against backup `zpool`; that would weaken `zpool/trading-backups compression=zstd`.
...
- Confirm disk retention defaults before first bring-up. Compose caps Docker stdout/stderr with `DOCKER_LOG_DRIVER=local`, `DOCKER_LOG_MAX_SIZE=50m`, and `DOCKER_LOG_MAX_FILE=5`; file logs rotate daily or at `maxsize 50M`, keep 10 compressed rotations with `maxage 21`, and cover `/app/logs`, `/auxpool/trading/runtime/logs`, `/var/lib/trading/logs`, `/opt/trading/app/logs`, ingestion stdout/stderr logs, boot stderr logs, and the diagnostics-only operator-AI JSONL log. Local base-backup retention is two base backups plus a WAL cushion; configure `TS_BASE_BACKUP_OFFSITE_CMD` for archival history.
...
- Enforce host memory-pressure hardening before runtime bring-up on `bart` and equivalent ZFS hosts. Run the read-only diagnostic first: `python -m engine.runtime.memory_pressure --json --required`. It must report `status=pass`, about 123 GiB RAM, at least 48 GiB total swap, at least 32 GiB zram, at least 16 GiB managed `/swapfile-trading`, `vm.swappiness=10`, and `zfs_arc_max=48 GiB`. A legacy 512 MiB `/swapfile` is `memory_pressure_total_swap_below_policy` and is not production-ready. Then run `sudo bash ops/server/memory_pressure_hardening.sh install` and `sudo bash ops/server/memory_pressure_hardening.sh verify`. Production preflight sets `PREFLIGHT_REQUIRE_MEMORY_PRESSURE_POLICY=1`, and `/api/health.memory_pressure` must stay `ok=true`. The managed target is persisted through sysctl.d, systemd units, and modprobe.d. See [MEMORY_PRESSURE_RUNBOOK.md](MEMORY_PRESSURE_RUNBOOK.md).
...
- `deploy/bin/apply_host_hardening.sh`
...
- `deploy/systemd/trading-restore-drill.service`
- `deploy/systemd/trading-restore-drill.timer`
...
- `ops/server/disk_remediation.sh`
- `ops/server/zfs_tuning.sh`
...
- Run `python tools/validate_dependency_lock.py --strict` after dependency manifest or lock changes.
...
- Run `python tools/pyright_money_path_gate.py` for changes in live data, execution, risk, runtime gate/storage, or portfolio/model-governance paths. The ratcheted baseline and staged package dates are documented in [PYRIGHT_TYPE_GATE.md](PYRIGHT_TYPE_GATE.md).
- Run `python tools/validate_repo.py` before merge for the full deterministic validation set. It runs the pyright money-path gate, the dashboard route-handler registration contract, fails separately on pytest collection before pytest execution, and its default startup graph check isolates itself from local Postgres/Timescale and Redis settings; use live validation when those services must be exercised.
...
- Keep `msgpack` installed from the locked runtime dependency profile and leave `CACHE_CODEC_ALLOW_JSON_FALLBACK=0` in production. Production startup gates and production preflight fail closed without msgpack because cache envelopes and Redis live snapshots are required to use the binary codec; the JSON fallback is only a local development escape hatch when explicitly enabled. Verify `get_live_cache_snapshot()` reports `redis_write_path=evalsha_lua_msgpack`, increasing `redis_evalsha_attempts`, and no fallback reason under load.
- For systemd hosts, keep dependency URLs passwordless and use credential-name env vars such as `LIVE_CACHE_REDIS_PASSWORD_SECRET=redis_password`, `OBJECT_STORE_ACCESS_KEY_SECRET=object_store_access_key`, `OBJECT_STORE_SECRET_KEY_SECRET=object_store_secret_key`, `DASHBOARD_API_TOKEN_SECRET=dashboard_api_token`, and `OPERATOR_API_TOKEN_SECRET=operator_api_token`; the unit must load those names with `LoadCredentialEncrypted=` and should use `UnsetEnvironment=` to strip stale inline object-store aliases from `EnvironmentFile=` input.
- Treat a secret-source preflight failure as a hard production blocker. `PROD_LOCK=1`, supervised production, `ENV=prod`, or live mode rejects inline values for broker/data/object-store/database/API credentials in process env and in repo-local `.env`, `.env.*`, `deploy/compose/.env`, `deploy/compose/.env.*`, and `deploy/env/trading.env`. Use `*_FILE`, `*_SECRET`, Docker Compose secrets, systemd credentials, or strict root/service-owned files instead. Required file-backed secrets, including `/run/secrets/*`, must be present, readable, non-empty, and not placeholders such as `/dev/null`.
...
There should be no `4001->4001` entry. In the preferred LAN contract, only the dashboard host publish may be non-loopback after the token and exposure acknowledgement are set. Put the dashboard behind an authenticated VPN or TLS reverse proxy for remote operators; keep Timescale, Redis, MinIO, and the operator sidecar off LAN-facing interfaces unless there is a reviewed design.
- Confirm passive host listeners before calling LAN exposure clean:
...
`prod_preflight.py` fails on wildcard/direct `:4001` and warns on unexpected wildcard listeners for `:4000`, `:7002`, and `:5201` so an operator can identify stray services without printing secret values.
- Run the repeatable safe/shadow production readiness gate before pre-deploy or live-enablement signoff:

```bash
python tools/production_readiness_gate.py
```

The gate boots a bounded safe/sim `start_all.py` stack on dashboard `:8000` and operator `:4001` with generated file-backed tokens, `BROKER=sim`, `EXECUTION_MODE=safe`, `ENGINE_MODE=safe`, `DISABLE_LIVE_EXECUTION=1`, and the global kill-switch hold. It does not clear or relax any live-safety setting. It fails closed with a non-zero exit if any invariant regresses: safe boot exposes both ports with zero credential-related startup crashes; `/api/system/kill_switches` is available; `/api/broker/config` reports broker `sim`; `/api/execution/barrier` reports `mode=safe`, `allowed=false`, and `real_trading_allowed=false`; `/api/operator/readiness_evidence` targets `sim/safe`; `/api/db/health` returns HTTP 200 with `ok=true`; a started `provider_monitor` write job records a heartbeat and shows zero `SET LOCAL $1` errors; `/api/terminal/order` is blocked with HTTP 403, `reason_code=disable_live_execution_env`, and `gate.mode=safe`; API read-back sees the persisted heartbeat; direct runtime `SIGTERM` exits inside the bounded deadline without `SIGKILL`; and dashboard/operator listeners close after cleanup. `python tools/validate_repo.py --pre-deploy` includes this readiness gate, so a failed readiness invariant is a pre-deploy blocker.
- Safe/sim feedless startup is allowed to serve as `WARMING_UP` or `DEGRADED` when no first price tick arrives, but only while `ENGINE_MODE=EXECUTION_MODE=OPERATOR_MODE=safe`, live execution remains disabled, and no live broker/feed is enabled. The operator records the reachable dashboard as supervision-ready for that safe attempt instead of restarting it for missing market data. Do not accept this degraded-serving state for live enablement.
...
- Confirm `GET /api/operator/service_status` and `GET /api/operator/support_snapshot` through the dashboard return redacted diagnostics and do not show unresolved startup failures. Sensitive dashboard GETs such as `/api/system/config`, `/api/operator/logs`, `/api/operator/support_snapshot`, and `/api/terminal/positions` require `X-API-Token` in production/live or remote-bind deployments; query-string `token` auth is rejected in production/live or on non-loopback binds.

## 5. Data-Source And Secret Checks

- Use `ui/data_sources.html` as the source-of-truth setup surface for provider credentials and source-specific settings. The page must render provider guidance, docs/signup links, plan notes, field help, required/optional state, placeholders, validation hints, and safety warnings from `GET /api/data_sources`; do not maintain separate provider setup copy in browser JavaScript.
- Confirm source mutation validation is enforced by [services/data_source_manager.py](../services/data_source_manager.py): unknown submitted credential/setting fields are rejected, optional format regex checks run before persistence, and validation errors name only field identifiers rather than submitted values.
...
- Confirm `GET /api/data_sources` reports paid feeds with absent or zero-byte credential files as `status=needs_credentials` with `missing_credential_env_vars`, not as enabled, healthy, blank, or crashed. RSS and other keyless feeds may remain enabled because they do not require provider secrets.
- Confirm data-source log redaction cleanup has run after upgrades that touch source credentials. The data-source manager records `runtime_meta.data_source_logs_sqlite_redaction_v1=1` after SQLite cleanup and, when Timescale telemetry is configured and reachable, `runtime_meta.data_source_logs_timescale_redaction_v1=1` after mirrored-row cleanup. `/api/data_sources/logs` must show `[REDACTED]` for credential-bearing detail keys and must not expose provider secrets.
- Confirm production secret inventory reports only key names and file locations, never values:
  ...
  PY
  ```
  Any key returned from `.env`, `.env.*`, `deploy/compose/.env`, `deploy/compose/.env.*`, or `deploy/env/trading.env` must be removed before production/live start.
- Replace existing local credentials with approved sources:
  ...
  - For file-backed deployments, write one secret per file with mode `0600` and an owner that the consuming service can read. For host-process services running as `trading`, that means service-owned files such as `trading:trading 0600`; root-owned files should be delivered through systemd credentials instead. Docker `/run/secrets/*` mounts may use Docker's normal file mode, but they still must be readable and non-empty. Use names such as `dashboard_api_token`, `operator_api_token`, `timescale_password`, `redis_password`, `minio_root_user`, `minio_root_password`, `polygon_api_key`, `tradier_api_token`, `openai_api_key`, `alpaca_key_id`, and `alpaca_secret_key`.
  - Replace inline variables with pointers: `DASHBOARD_API_TOKEN_FILE`, `OPERATOR_API_TOKEN_FILE`, `TS_PG_PASSWORD_FILE` or `TIMESCALE_PASSWORD_FILE`, `REDIS_PASSWORD_FILE`, `OBJECT_STORE_ACCESS_KEY_FILE`, `OBJECT_STORE_SECRET_KEY_FILE`, `POLYGON_API_KEY_FILE`, `TRADIER_API_TOKEN_FILE`, `OPENAI_API_KEY_FILE`, `ALPACA_KEY_ID_FILE`, and `ALPACA_SECRET_KEY_FILE`. Disabled optional providers may keep placeholder compose secrets for dependency-only bring-up; setting `POLYGON_REST_ENABLED=1`, `POLYGON_WS_ENABLED=1`, `TRADIER_ENABLED=1`, `OPENAI_ENABLED=1`, or any broker selector to `alpaca` makes the matching secret source mandatory.
  - Keep Postgres and Redis connection strings passwordless. Use `TS_PG_DSN=host=... user=... dbname=...`, `TIMESCALE_DSN=postgresql://user@host:5432/db`, and `LIVE_CACHE_REDIS_URL=redis://redis:6379/0`; the runtime attaches passwords from the matching secret file/provider at connection time. On systemd hosts, `CREDENTIALS_DIRECTORY` plus `LoadCredential` keeps Postgres `*_PASSWORD_SECRET` lookups first. On non-systemd file-backed hosts, unavailable systemd Postgres secret lookups are skipped before file fallback so successful starts do not emit `credentials_directory_missing` warnings.
...
- Use `/api/broker/config`, `/api/broker/test_connection`, and `/api/broker/audit` as the broker configuration control plane. In `EXECUTION_MODE=safe` / `ENGINE_MODE=safe`, `/api/broker/test_connection` does not open live broker sockets for non-`sim` brokers; it returns a safe-mode skipped result that is not eligible for non-`sim` activation. Do not activate a non-`sim` broker until the connection test passes freshly for the same broker outside safe mode and the audit rows show the expected operator action. `BROKER_CONNECTION_TEST_MAX_AGE_S` controls the backend activation freshness window and defaults to 24 hours.
...
- Confirm `ENGINE_MODE=live` and `EXECUTION_MODE=live`; environment mode alone does not arm execution. Mode parsing is fail-closed: only canonical `safe`, `paper`, `shadow`, and `live` values, plus documented safe/paper aliases, are accepted. Unknown, malformed, whitespace-only, or stale API/config-derived mode values surface as `invalid_execution_mode` and block the execution gate and live broker boundary.
...
- Confirm live AI safety is clean before live capital is armed. `DECISION_ENGINE_ENABLED=1`, `DECISION_MIN_CONFIDENCE`, `DECISION_MIN_ABS_PREDICTION`, `UNCERTAINTY_SIZING_PRODUCTION_POLICY`, `UNCERTAINTY_HIGH_THRESHOLD`, `UNCERTAINTY_HARD_THRESHOLD`, `UNCERTAINTY_MAX_AGE_MS`, `OOD_SUPPRESS_THRESHOLD`, and `OOD_HARD_THRESHOLD` must be explicit. If `CONFORMAL_RISK_CONTROL_ENABLED=1`, `CONFORMAL_RISK_TARGET` and `CONFORMAL_RISK_MIN_SAMPLES` must also be explicit. Missing or disabled values fail `live_trading_preflight()` and suppress live risk-increasing orders through the execution policy.
- Confirm the live model resolves without fallback for `LIVE_AI_PREFLIGHT_SYMBOLS` and `LIVE_AI_PREFLIGHT_HORIZONS_S`, and every resolved live model has a readable artifact alias, SHA, or path. Silent fallback from the requested model to another model, env-default `MODEL_NAME` serving without a governed champion unless `LIVE_ALLOW_ENV_DEFAULT_MODEL=1`, missing artifacts, online-model dummy zero predictions, `RL_ALLOW_FALLBACK_AGENT=1`, or RL/LLM/advisory source metadata on live orders are hard blockers.
- Keep `FX_LIVE_TRADING_ENABLED=0` unless live FX order authority has explicit owner approval. FX research/feed enablement is separate from live order authority: unset or false values block live FX routing in `broker_router` and direct IBKR CASH/IDEALPRO submit helpers, while `live_trading_preflight().asset_class_live_enablement` reports the FX/crypto/futures/options live-enable posture for operator review.
- Keep `OPTIONS_INSTRUMENTS_MODE=shadow` for production. Options chain ingestion, OCC/Polygon contract parsing, `asset_class=OPTION` classification, and options-derived features are not live options execution support. Live options orders are blocked in production code by runtime config validation, `live_trading_preflight().options_instruments`, the broker router, and direct Alpaca/IBKR adapter checks until a reviewed live options adapter exists and the Greeks, liquidity, bid/ask, assignment/exercise, expiration-risk, margin-impact, broker-support, position-limit, and kill-switch controls are implemented and enabled.
- Confirm `/api/health.options_ingestion.credentials_configured` matches the intended options-data posture. With no `POLYGON_API_KEY`/`TRADIER_API_TOKEN` resolved, `options_ingestion.detail=options_provider_unconfigured` is expected and non-blocking. If either credential is configured in paper/live, stale or empty `options_chain_v2` ingestion is degraded and production preflight blocks with `options_chain_stale_despite_credentials`.
- Confirm `/api/health.options_ingestion.data_quality` is not green by accident before trusting options-derived features. `available=false` means no fresh chains or no active options universe, while `degraded=true` identifies coverage, bid/ask, greeks, or IV-sanity failures. Runtime metrics are emitted under `options.dq.*`, and a throttled normalized options event with `event_kind=options_data_quality_degraded` records threshold failures through the normalized `events` contract (`raw_payload`, `derived_features`, `source_id`, `dedupe_hash`, and event-key conflict support). `OPTIONS_DQ_MIN_COVERAGE`, `OPTIONS_DQ_MIN_COMPLETENESS`, and `OPTIONS_IV_SANITY_MAX` are operator-tunable sanity thresholds; they do not enable `USE_OPTIONS_FEATURES`.
- Confirm runtime options greek caps match the intended shadow-risk posture. `portfolio_risk_engine` aggregates OPTION `delta`, `gamma`, `theta`, and `vega` from `options_chain_v2` using the canonical OCC multiplier and blocks projected books only when `PORTFOLIO_RISK_USE_OPTIONS_GREEK_LIMITS=1` and a positive cap is configured. Empty or non-positive `OPTIONS_MAX_PORTFOLIO_DELTA_ABS`, `OPTIONS_MAX_PORTFOLIO_GAMMA_ABS`, `OPTIONS_MAX_PORTFOLIO_VEGA_ABS`, `OPTIONS_MARGIN_IMPACT_MAX_FRACTION`, and `OPTIONS_MAX_POSITION_CONTRACTS` values fail open for that cap; this is portfolio-risk accounting and does not enable live options routing.
- Keep option lifecycle processing disabled unless explicitly exercising shadow/paper settlement. `OPTIONS_LIFECYCLE_ENABLED=1` lets `broker_sim` auto-close, roll-close, expire, exercise, or assign simulated option positions through synthetic broker fills, but it is still not live order authority and does not flip any readiness gate. The simulator applies a deterministic reference model: expiry intrinsic is `max(0, S-K)` for calls and `max(0, K-S)` for puts times contracts times multiplier; long exercise and short assignment are cash-settled in broker-sim rather than root-specific physical delivery; American optimal early exercise is not modeled. Pin-risk positions within `OPTIONS_PIN_RISK_BAND_ABS` are flagged deterministically rather than randomly assigned.
...
- Confirm `CONFORMAL_MODE`, `CONFORMAL_RISK_CONTROL_MODE`, `OOD_MODE`, and `UNCERTAINTY_SIZING_MODE` match the intended rollout. `CONFORMAL_MODE=gate_and_size` applies interval-crosses-zero suppression and wide-interval size reduction; CQR is used only when model outputs expose lower/median/upper or quantile forecasts and enough historical quantile calibration exists, otherwise symmetric residual intervals remain the fallback; `CONFORMAL_RISK_CONTROL_MODE` only recommends `log_only`, `size_compress`, `suppress`, or `hard_block` actions that portfolio/risk/execution gates must enforce; `OOD_MODE=suppress` applies OOD compression/blocking; `UNCERTAINTY_SIZING_MODE=enforce` applies model-intent and epistemic uncertainty sizing outside live mode.
...
- Confirm at least one off-box critical-alert delivery channel is configured before live enablement. `prod_preflight.py` runs `_notification_channel_gate()`, which reads `get_notification_channel_status()` from `engine/runtime/alerts_notify.py` and fails closed in live mode (`required` follows live mode) with `alert notification channel gate invalid` when no channel is enabled. Configure either SMTP with both `EQ_CRIT_SMTP_HOST` and `EQ_CRIT_EMAIL_TO`, or set `EQ_CRIT_WEBHOOK_URL` to a Slack, Discord, or generic HTTPS webhook, in `deploy/env/trading.env` or the host secret/env source. Safe/non-live runs only warn, but live go-live is blocked until at least one channel is enabled; the `notification_channels` block in `prod_preflight.py --json` reports `enabled_channels` and `configured_channels`. See [FAILURE_MODES.md](FAILURE_MODES.md) "Critical Alert Delivery".
...
- Keep live-risk thresholds explicit in `/etc/trading/trading.env`; do not leave the live-required values blank. The systemd template now carries the same go-live defaults as the compose template: `PORTFOLIO_RISK_USE_MONTE_CARLO=1`, `PORTFOLIO_RISK_MC_REQUIRED_IN_LIVE=1`, `MODEL_AWARE_KILL_SWITCH=1`, non-zero `PORTFOLIO_RISK_MC_*_BLOCK` thresholds, `PORTFOLIO_RISK_VOL_HARD_BLOCK=0.12`, `KILL_SWITCH_MODEL_MAX_DRAWDOWN=5000`, and `KILL_SWITCH_MODEL_MAX_CONSECUTIVE_LOSSES=4`. Runtime import code treats blank optional numeric kill-switch env values as unset so validators can produce actionable diagnostics, but `config_schema.py` still rejects missing live-required thresholds.
...
- Confirm at least one paid equity market-data rail is provisioned and passing before paper/live arming. yfinance is a fallback research feed and does not satisfy `live_market_data_ok` or `prod_preflight.py --json`. Provision Polygon and/or IBKR through file-backed secrets such as `/etc/trading/secrets/POLYGON_API_KEY` with `POLYGON_API_KEY_FILE` set and mode `0600`, then validate the source through `POST /api/data_sources/test` and require `status=pass`. Provision related paid feeds for the intended launch scope, including news, earnings, and options credentials, before enabling their jobs.
...
- Keep `RUNTIME_SHUTDOWN_HARD_DEADLINE_S` inside the service-manager stop window. `start_system.py` and the serving dashboard SIGTERM/SIGINT handler mark clean shutdown, terminate startup-owned ingestion when applicable, run `runtime_shutdown()`, then join leftover non-daemon threads only until that hard deadline; if the deadline is exceeded the handler logs the outstanding step, flushes logs, and force-exits instead of waiting for lock release or background thread completion.
- Keep `RUNTIME_SHUTDOWN_DRAIN_DEADLINE_S` within the service-manager stop window. On whole-runtime shutdown, child jobs receive terminate first; supported supervisor/job-manager stop-all paths then run the async price writer and telemetry append-buffer drain under that bounded deadline, record a `runtime_shutdown_drain` event with residual risk, and only then SIGKILL children that are still alive. Treat `residual_dropped_rows>0` as possible telemetry loss; `residual_spooled_rows>0` means rows were retained in durable spool for replay.
- Keep `JOBS_MANAGER_LOCK_RELEASE_TIMEOUT_S` short enough for shutdown. Runtime job-lock release is best-effort during `stop_all()`; failures and timeouts are logged without blocking shutdown, so stale-lock reconciliation must rely on lock TTL/heartbeat recovery rather than an unbounded stop path.
...
- Keep Postgres base backups, WAL archive, backup pruning, and restore-drill timers configured through `ops/backup/` and the host systemd units. `ops/server/systemd/` is the canonical bootstrap source; `deploy/systemd/` also ships the weekly `trading-restore-drill` unit/timer for the deploy installer. The older `deploy/bin/backup_trading_db.sh` copies a SQLite file and is not sufficient for the current Postgres-backed runtime.
- Treat restores as part of operations: run the restore drill into a clean target weekly and keep the latest drill report with the backup evidence. Production restore-drill evidence freshness should stay within 604800-1209600 seconds (7-14 days), with shipped production examples using 604800 seconds.
- Before live promotion, run `ops/backup/backup_restore_evidence.sh` on the target server. Keep the timestamped report and `latest_backup_restore_evidence.json`; live preflight and model promotion guard fail closed when the JSON artifact is missing/invalid, when verified backup, WAL archive evidence, WAL archive target ownership/mode evidence, `pg_stat_archiver` evidence, restore drill freshness, restore duration, signature freshness, or signature verification violates the configured `BACKUP_EVIDENCE_*` policy. Live and production-required evidence paths require a valid HMAC-SHA256 signature even if an old env file still sets `BACKUP_EVIDENCE_REQUIRE_SIGNATURE=0`.
- Keep `trading-backup-evidence.service` as a bounded assertion gate:
  ...
  `TS_BACKUP_EVIDENCE_RUN_WAL_CATCHUP=0`. The minute timer reasserts and repairs
  the WAL archive target owner/group/mode, then fails non-zero for
  missing/stale/inaccessible base backup, WAL, `pg_stat_archiver`, WAL target, or
  restore-drill evidence instead of launching heavyweight recovery work.
- For Compose TimescaleDB, the WAL archive command runs inside the database container as `postgres` and writes to the `/var/backups/trading` bind mount. The backup root must be the ZFS dataset mounted at `/var/backups/trading` with permissions compatible with the container UID, for example `2750 70:trading` on the root/WAL target directories. `wal_archive.sh` fails if that root is not a mount, is not writable, or resolves to the container/root filesystem; it falls back to `sync -f`/`sync` when the Timescale image does not include `python3`.
- The Compose installer records `TS_BACKUP_WAL_TARGET_OWNER_UID=<container postgres uid>`,
  `TS_BACKUP_WAL_TARGET_GROUP=trading`, and
  `TS_BACKUP_WAL_TARGET_DIR_MODE=2750` for the evidence timer. The signed JSON
  must contain `wal_archive_target.status=pass`; when a repair was needed,
  `wal_archive_target.diagnosis` records a pre-repair `wal_archive.sh` probe as
  the expected archive owner, including `archive_command_probe_status`,
  `original_archive_command_failure_signature` such as
  `archive_dir_not_writable`, `original_archive_command_exit_code`, the observed
  `pg_stat_archiver` failure fields, and the `chown ...; chmod 2750 ...` fix.
...
`last_archived_time`; `pg_wal` bytes and `.ready` backlog are under the
configured `PREFLIGHT_PG_WAL_*` limits.
...
jq '{status,timeouts,base_backup,wal_catchup,wal_archive,wal_archiver,wal_archive_target,restore_drill,signature:{status,key_id,signed_at,payload_sha256}}' \
...
`wal_archive_target.status=pass`, `wal_archive_target.expected_owner_uid`
matches the Timescale image's `postgres` UID, `wal_archive_target.expected_group`
is `trading`, `wal_archive_target.expected_dir_mode=2750`,
...
`skipped` during normal timer health checks. This is deliberate: backlog drain is
operator-only because a large `.ready` queue can consume backup-dataset space.
Run `wal_archive_catchup.sh` explicitly after correcting the archive target or
restoring the backup dataset; the health gate should then prove the real
archiver by checking `pg_stat_archiver`.
...
- Store the backup evidence signing key outside the backup evidence directory. The focused installer creates `/etc/trading/backup_evidence.hmac.key` when missing for one-shot evidence generation, then installs the matching encrypted systemd credential as `backup_evidence_hmac_key`. The plaintext file is provisioning material only; runtime units must consume the encrypted credential with `BACKUP_EVIDENCE_HMAC_KEY_SECRET=backup_evidence_hmac_key` and `BACKUP_EVIDENCE_HMAC_KEY_FILE=`. Manual creation:
...
- Rotate the backup evidence key by replacing the encrypted `backup_evidence_hmac_key` credential on systemd hosts, or by updating `BACKUP_EVIDENCE_HMAC_KEY_FILE` and remounting the compose secret on Compose hosts. Generate fresh evidence with the new key and rerun preflight. Keep the previous key until fresh signed evidence and preflight verification both pass.
...
- Short-equity borrow / financing cost is an offline cost-realism term, not a broker fill cost. With the default config (`EQUITY_BORROW_COST_ENABLED` unset or `1`), `engine.strategy.net_after_cost_labels` synthesizes a stock-borrow cost for SHORT legs whose asset class is `EQUITY`/`US_EQUITY`, writes it to the existing `borrow_bps`/`borrow_cost`/`total_cost_bps` fields, and reduces short-equity `net_return` by the borrow return. LONG legs, non-equity asset classes, and flag-off runs stay on the legacy path; set `EQUITY_BORROW_COST_ENABLED=0` only for deliberate borrow-free baselines. `engine.strategy.cpcv` also charges per-period borrow carry for short-held equity intervals when `CPCV_BORROW_COST_ENABLED` is unset or `1` (default inherits `EQUITY_BORROW_COST_ENABLED`) using `CPCV_PERIOD_DAYS` as the rebalance-period length.
- The borrow schedule lives in `engine.strategy.borrow_cost_model`: default annual bps are `GC=30`, `MODERATE=75`, `HARD=300`, and `SPECIAL=1000`, keyed primarily from FINRA-style `days_to_cover` (`<2`, `<5`, `<10`, otherwise special). Override annual rates with `EQUITY_BORROW_BPS_PER_YEAR_JSON`, thresholds with `EQUITY_BORROW_DTC_THRESHOLDS_JSON`, and the no-signal floor bucket with `EQUITY_BORROW_DEFAULT_BUCKET`. Missing difficulty uses the default bucket rather than a free short; no new feature ids or storage columns are required.
...
- For TSFM challengers, run `tsfm_benchmark` only as a governed shadow benchmark (`USE_TSFM_BENCHMARK=1`). Chronos/TimesFM/Moirai/Toto/fake adapter outputs write PIT OOS rows, feature snapshots, artifact metadata, latency/resource summaries, shadow risk-input provenance for realized-volatility tasks, and `model_marketplace_scores.stage=shadow` rows with `score_source=model_oos_predictions`. A zero-shot TSFM win is not live-promotion evidence until the normal champion/challenger path has realized net-cost labels, replay approval, statistical evidence, and registry promotion.
- Keep graph/relational learning shadow-only. `USE_GRAPH_RELATIONAL_FEATURES=1` may materialize `graph.relational_v1.*` snapshots for research, and `GRAPH_CHALLENGER_BENCHMARK_ENABLED=1` may run the temporal heterogeneous graph challenger benchmark. The benchmark builds PIT-safe temporal samples from existing node-feature snapshots and graph relationship edges, persists graph artifacts, OOS rows, parity metadata, and `model_marketplace_scores.stage=shadow` rows, and reports graph-vs-node-only ablation evidence. Live model serving still rejects graph feature ids, graph predictions have `direct_trading_authority=false`, and both direct registry promotion and champion competition block graph candidates when graph metadata, PIT safety, train/serve parity, or snapshot availability is missing. Fully valid graph metadata is still non-promotable until a reviewed live graph gate replaces the shadow-only blocker.
...
- If `llm_event_extraction` is enabled for an offline batch, confirm `LLM_EVENT_EXTRACT_ENABLED=1` is deliberate, the selected provider credential resolves through `*_FILE`, `*_SECRET`, or the configured secret provider, and `LLM_EVENT_EXTRACT_MAX_COST_USD` plus `LLM_EVENT_EXTRACT_MIN_INTERVAL_MS` are set for the run. Review `llm_event_extraction_audit` for rejected JSON, future-document exclusions, prompt/model/source hashes, and bounded raw-response excerpts. Accepted rows may only project to `structured_doc_events_v1.*`, which remains `stage=shadow` with `direct_trading_authority=false`; they do not satisfy promotion evidence and cannot authorize orders.
- Keep `compute_drift` running in supervised production. It now refreshes `production_monitoring_metrics` for feature drift, prediction drift, missing feature rates, target/label drift after labels mature, calibration ECE, conformal coverage, conformal risk-control breaches, shadow-vs-live disagreement, and net-PnL degradation. Threshold breaches write `drift_retrain_events` rows with `action_taken` of `retrain_signal` or `shadow_review_signal`; they do not promote or live-ready a model.
...
- Keep the configured runtime log directory available for runtime and operator log tails. Local defaults use `var/log/`; deployment overrides such as `/app/logs` or `/var/lib/trading/logs` still take precedence. On systemd installs, `deploy/bin/service_ctl.sh logs` reads `/var/lib/trading/logs/{engine,operator,upgrade}.log` first and falls back to journald only when no file sink is available.
- After installing or updating `deploy/systemd/trading-engine.service` and `deploy/systemd/trading-operator.service`, run `sudo systemctl daemon-reload` and restart both services. On systemd 259, inactive services report `WatchdogUSec=infinity` even when the unit file contains `WatchdogSec=60s`; the go-live verification must show active services with finite watchdog budgets plus the expected `MemoryMax`, `OOMScoreAdjust`, and `TimeoutStartUSec=5min` values. The operator prefers the native `unix-dgram` notify path and falls back to `/usr/bin/systemd-notify`, so keep `npm ci` successful and verify the unit is restarted after dependency refreshes.
