...
Execution job entrypoint for `compute_exec_labels_from_fills`.
"""
...
LOG = get_logger("engine.execution.jobs.compute_exec_labels_from_fills")
...
def _warn_nonfatal(code: str, error: BaseException, *, once_key: str | None = None, **extra: object) -> None:
    ...
    log_failure(
        ...
        component="engine.execution.jobs.compute_exec_labels_from_fills",
        extra=extra or None,
        persist=False,
    )
    if once_key:
...
def _cost_bps_from_trade(trade: dict, px_in: float, px_out: float, side: int) -> dict:
    ...
    # Convert fees into bps relative to entry notional when available.  Older
    # trade payloads lack quantity, so retain the one-share fallback.
    fees_bps = 0.0
    try:
        notional = float(trade.get("entry_notional") or 0.0)
        if notional <= 1e-12:
            qty = abs(float(trade.get("qty") or trade.get("entry_qty") or 0.0))
            notional = qty * float(pin)
        if notional <= 1e-12:
            notional = float(pin)
        fees_bps = float(fees_total) / float(notional) * 10000.0
...
def main():
    if os.environ.get("ENGINE_SUPERVISED") != "1":
        print("compute_exec_labels_from_fills must be launched by supervisor")
        raise SystemExit(1)
    ...
    try:
        ...
        for eid, sym, horizon_s, ts_ms in rows:
            ...
            try:
                ...
                raw_forward_ret = (float(px_out_use) / float(px_in)) - 1.0
                artifact = build_net_after_cost_label(
                    ...
                    metadata={
                        "labels_exec_source": "broker_fills_v2",
                        "entry_price": float(px_in),
                        "exit_price": float(px_out_use),
                        "exit_target_ts_ms": int(exit_ts),
                        "m2m": m2m_ctx,
                        "trade": trade,
                        "alpha_decay": decay,
