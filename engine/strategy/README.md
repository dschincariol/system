...
- Prediction-market alternative data:
  `prediction_market_macro_v1.*` and `prediction_market_event_v1.*` feature ids
  are registered through
  [feature_registry.py](feature_registry.py), resolved by
  [model_feature_snapshots.py](model_feature_snapshots.py), and enforced by
  [feature_pit.py](feature_pit.py). They are shadow-only, carry
  `direct_trading_authority=false`, and are omitted from the default unified
  serving feature set unless a model explicitly requests them for research,
  backfill, or challenger evaluation. Regulated event-contract ids cover mapped
  macro, energy, climate/weather, FX/rates, equity-index, and commodity
  probabilities from ForecastEx CSVs and optional read-only IBKR market data.
- Crypto derivatives alternative data:
  `deribit_crypto_derivatives_v1.*` feature ids are registered through
  [feature_registry.py](feature_registry.py), resolved by
  [model_feature_snapshots.py](model_feature_snapshots.py), and enforced by
  [feature_pit.py](feature_pit.py). They are shadow-only Deribit public
  BTC/ETH/SOL volatility and positioning signals: IV rank, short-dated IV,
  skew proxy, term structure, put/call open-interest ratio, futures/perp basis,
  funding pressure, volume shock, and volatility-regime flags. The resolver is
  crypto-only unless a deliberate crypto-equity mapping is explicitly enabled,
  and any live use must first pass out-of-sample, net-after-cost, PIT,
  deconfounded, and production-readiness evidence gates.
- Sportsbook odds research:
  `sports_odds_sector_v1.*` feature ids are registered through
  [feature_registry.py](feature_registry.py), resolved by
  [model_feature_snapshots.py](model_feature_snapshots.py), and enforced by
  [feature_pit.py](feature_pit.py). They are shadow-only, research-only,
  disabled by default, and resolved only when explicitly requested. The resolver
  requires exact sports/event category mapping rows, blocks broad-market
  symbols through the narrow allowlist, uses no-vig probabilities only, excludes
  resolved rows from pre-resolution snapshots, and carries
  `direct_trading_authority=false`. Production feature use is NO-GO until a
  narrow approved mapping with complete owner, rationale, mapping-version,
  approver, timestamp, approval-reason, and `approved_for_promotion` metadata
  has persisted `sportsbook_odds_promotion_evidence` proving incremental
  out-of-sample, net-after-cost, PIT-safe, deconfounded, provider-ready, and
  production-ready edge. `promotion_guard.assess_challenger` calls the
  sportsbook GO gate whenever challenger feature ids include
  `sports_odds_sector_v1.*`, including comma-delimited feature-id strings from
  configs.
