...
DEFAULT_OUTPUT_DIR = Path("data") / "retraining"
...
def _feature_snapshot_for_row(
    ...
    *,
    ...
    try:
        ...
        LOG.warning(
            "retraining_feature_snapshot_failed symbol=%s anchor_ts_ms=%s",
            str(symbol),
            int(anchor_ts_ms),
            exc_info=True,
        )
...
def extract_training_data(config: RetrainingPipelineConfig) -> dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            LOG.warning("retraining_pipeline_close_failed", exc_info=True)
...
def trigger_retraining_jobs(job_names: Sequence[str]) -> list[dict[str, Any]]:
    ...
    for job_name in job_names:
        ...
        try:
            ...
            LOG.warning("retraining_job_start_failed job=%s error=%s", name, exc, exc_info=True)
