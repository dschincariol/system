...
class RuntimeConfigSchemaTests(unittest.TestCase):
    ...
    def test_prod_preflight_runtime_config_gate_accepts_explicit_live_contract(self) -> None:
        ...
        joined_notes = "\n".join(notes)
        self.assertIn("workload_profile=live", joined_notes)
        self.assertIn("allow_training=0", joined_notes)
        self.assertIn("runtime hardware ok", joined_notes)
        self.assertIn("dependency_profile=cpu", joined_notes)
        self.assertIn("torch_device=cpu", joined_notes)
        self.assertIn("live risk thresholds ok", joined_notes)
        self.assertIn("promotion observation governance ok", joined_notes)
        self.assertIn("non_bypassable=1", joined_notes)
        self.assertIn("live environment contract ok", joined_notes)
        self.assertIn("execution_arming_audited=1", joined_notes)
    ...
    def test_live_risk_acceptance_override_allows_audited_exception(self) -> None:
        ...
        self.assertIn("runtime hardware ok", joined_notes)
        self.assertIn("torch_device=cpu", joined_notes)
        self.assertIn("live risk thresholds override accepted", joined_notes)
        self.assertIn("id=RISK-123", joined_notes)
        self.assertIn("promotion observation governance ok", joined_notes)
        self.assertIn("non_bypassable=1", joined_notes)
        self.assertIn("live environment contract ok", joined_notes)
