...
def test_check_coverage_fails_engine_strategy_below_floor(
    tmp_path: Path,
    capsys,
) -> None:
    pyproject = tmp_path / "pyproject.toml"
    ...
    payload["files"]["engine/strategy/floor_guard.py"] = {
        "summary": {
            "covered_lines": 5,
            "num_statements": 10,
            "covered_branches": 0,
    ...
    payload["totals"]["num_statements"] += 10
    _write_pyproject(
        pyproject,
        minimum=0.0,
        package_minimums={"engine/strategy": 80.0},
    )
    ...
    captured = capsys.readouterr()
    assert exit_code == 1
    assert "engine/strategy" in captured.out
    assert "FAIL" in captured.out
    assert "engine/strategy 50.00% is below 80.00%" in captured.err
...
@pytest.mark.parametrize(
    ("pytest_exit_code", "gate_exit_code", "expected_exit_code"),
    [
        (0, 1, 1),
        (1, 0, 1),
        (1, 2, 2),
...
def test_run_coverage_returns_max_pytest_and_gate_severity(
    tmp_path: Path,
    monkeypatch,
    ...
    def fake_check_coverage(
        checked_json: Path,
        checked_config: coverage_gate.CoverageGateConfig,
    ) -> int:
        assert checked_json == coverage_json
        assert checked_config == config
        return gate_exit_code

    monkeypatch.setattr(coverage_gate.subprocess, "run", fake_run)
    monkeypatch.setattr(coverage_gate, "check_coverage", fake_check_coverage)

    exit_code = coverage_gate.run_coverage(config, [])

    metadata = json.loads((tmp_path / coverage_gate.COVERAGE_GATE_METADATA).read_text())
    assert exit_code == expected_exit_code
    assert metadata["pytest_exit_code"] == pytest_exit_code
