...
| `corporate_actions` | Regular | cleanup=n/a | low | source id upsert and symbol/ex-date availability-time label and hygiene reads | append-only PIT split and cash-dividend calendar keyed by source record id |
...
| `futures_continuous_bars` | Hypertable | time=ts_ms; chunk=1 day; compress=7 days; retain=3 years; segmentby=continuous_symbol | medium | continuous-symbol/time scans for return-safe model inputs and dashboard views | ratio-adjusted futures continuous bars used by labels, features, and backtests |
| `futures_contract_bars` | Hypertable | time=ts_ms; chunk=1 day; compress=7 days; retain=3 years; segmentby=contract | high | contract/time scans for roll detection, continuous-series construction, and dashboard curves | raw futures per-contract OHLCV and open-interest bars consumed by roll construction |
| `futures_roll_calendar` | Regular | cleanup=n/a | low | root/time lookup for roll-aware labels, execution blocking, and dashboard display | derived futures roll calendar keyed by root and roll timestamp |
| `futures_roll_yield` | Hypertable | time=ts_ms; chunk=1 week; compress=30 days; retain=3 years; segmentby=root | medium | latest and historical root/time scans for futures features and dashboard views | derived futures roll-yield feature series keyed by root and time |
...
| `graph_challenger_runs` | Regular | cleanup=retain with model evidence artifacts | low | latest graph challenger runs by model name and creation time | shadow graph-relational challenger run manifests and promotion-readiness diagnostics |
...
| `gov_symbol_sector_map` | Regular | cleanup=n/a | low | symbol sector lookup | symbol-to-sector map for government-flow conditioning and portfolio-risk sector budgets; populated from explicit env rows, reference tables, or the checked-in equity sector seed |
...
| `garch_vol_forecasts` | Hypertable | time=ts_ms; chunk=1 week; compress=30 days; retain=3 years; segmentby=symbol | medium | latest forecast by (symbol, model_type, ts_ms) and walk-forward validation windows | point-in-time GARCH-family volatility forecasts used by sizing and Monte Carlo risk inputs |
...
| `llm_event_extraction_audit` | Regular | cleanup=n/a | low to medium | latest audit rows by status/source document and forensic lineage review | append-only LLM extraction acceptance/rejection ledger with prompt, model, source hash, cost, and PIT guard lineage |
| `llm_extracted_events` | Regular | cleanup=n/a | low to medium | source-document audits and symbol availability-window PIT feature projection | validated structured LLM event rows keyed by source document, evidence span, schema, prompt, and model lineage |
...
| `options_predictor_shadow` | Hypertable | time=ts_ms; chunk=1 week; compress=30 days; retain=3 years; segmentby=underlying | medium | underlying/time scans for shadow options forecast review and promotion evidence | shadow options predictor forecasts and gated structure-intent evidence |
...
| `tsfm_benchmark_rows` | Hypertable | time=ts_ms; chunk=1 week; compress=30 days; retain=3 years; segmentby=symbol,family | medium | symbol/task/family walk-forward benchmark comparisons and promotion evidence review | point-in-time TSFM challenger and baseline OOS benchmark rows with feature/provenance snapshots |
| `tsfm_benchmark_runs` | Regular | cleanup=retain with model evidence artifacts | low | latest benchmark run by run_id/status and governance evidence review | governed time-series foundation-model benchmark run manifests and governance summaries |
| `tsfm_risk_inputs` | Hypertable | time=ts_ms; chunk=1 week; compress=30 days; retain=3 years; segmentby=symbol,adapter | medium | latest shadow risk-input candidate by symbol/adapter/time | shadow TSFM volatility forecasts registered as risk-input candidates with explicit provenance |
