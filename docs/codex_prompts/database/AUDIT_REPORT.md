...
the Linux platform contract. Conducted 2026-05-02. Remediation pass
2026-05-03.
...
| P1-3 — `linux_only` marker inert | ✅ Fixed: `pytest_runtest_setup` honours Linux-only skips |
| P1-4 — CI matrix Linux-only | ✅ Fixed: `.github/workflows/validate.yml` runs on `ubuntu-latest` |
...
**Final consolidated audit bundle** (Linux dev host, no live Postgres):
...
- 2 × `test_secrets_provider_systemd.py` — `linux_only`, no systemd credential directory
...
2. The Linux platform contract in `CROSS_PLATFORM.md` was audited
   against the implementation (platform helpers, env-var defaults,
   path handling, CI matrix, pytest markers).
3. The Python test suite was executed on a Linux host to surface real
   failures, not just structural compliance.
...
- **7 of 10** Linux platform compliance criteria met.
...
| 06 | PgBouncer + observability | 8/9 | 🟡 PARTIAL — dedicated `docs/Observability.md` missing (content lives in `docs/archive/Database_Production_Plan.md`) |
...
| 09 | Secrets via systemd-creds | 8/8 | ✅ PASS — Linux systemd-creds provider, plaintext dev fallback, rotation runbook |
| ⊥ | Linux platform compliance | 7/10 | 🟡 PARTIAL — CI is Linux-only; pytest marker skip logic missing |
...
| P1-3 | `linux_only` marker declared but no skip logic; tests run on the wrong platform | `tests/conftest.py:14-16` | Add `pytest_runtest_setup` hook that calls `pytest.skip` when `sys.platform` does not match the marker. | 5 min |
| P1-4 | CI must remain Linux-only | `.github/workflows/validate.yml:9` | Keep the validation runner on `ubuntu-latest` and run the SQLite contract there. | 15 min |

### P2 — polish

| # | Gap | File / Location | Recommended fix | ETA |
|---|---|---|---|---|
| P2-1 | `docs/Observability.md` missing; content lives in `docs/archive/Database_Production_Plan.md` | `docs/` | Create `docs/Observability.md` that consolidates: pg_stat_statements queries, slow-log discipline, the snapshotter behaviour, and the Grafana panel pointers. Link it from the `database/06_*.md` prompt and from the production checklist. | 30 min |
...
- No OS-specific secret API remains in core code beyond Linux systemd-creds.
...
```bash
# Linux platform contract (no DB needed)
...
If all three batches pass on Linux CI, the
...
They block a clean local dev experience (P1-2 in
...
2. Verify the test suite is green on the Linux dev host with no
