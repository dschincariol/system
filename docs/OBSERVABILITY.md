...
| Ingestion writer diagnostics | `engine/runtime/ingestion_runtime.py`, `engine/runtime/async_writer.py`, `engine/runtime/telemetry_append_buffer.py`, `engine/runtime/timescale_client.py`, `engine/runtime/storage_pg_prices.py` | Queue depth, flush latency, dropped rows, retry counts, and DB write duration for ingestion writers. The ingestion runtime embeds these snapshots in its heartbeat and `runtime_meta.ingestion_state`; `/api/health` also exposes `async_price_persistence`, `pg_price_storage`, `timescale`, and `telemetry_append_buffer`. |
...
- `/api/health`: inspect `async_price_persistence.queue_depth`, `async_price_persistence.dropped_rows`, `async_price_persistence.dead_letters`, `pg_price_storage.last_write_duration_ms`, `timescale.queue_depth`, `timescale.metrics.backpressure_count`, `timescale.metrics.last_db_write_duration_ms`, and `telemetry_append_buffer.buffered_rows`.
- `runtime_meta.ingestion_state.writer_diagnostics`: inspect `degraded_reasons`; queue pressure, drops, dead letters, Timescale backpressure, and price-storage errors mark ingestion degraded.
...
6. `sudo /opt/trading/app/ops/backup/accounting.sh`
