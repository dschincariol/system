...
- Provide a credential-encryption root with `DATA_SOURCE_MASTER_KEY` or `DATA_SOURCE_MASTER_KEY_FILE`. Production/live preflight accepts only canonical base64 text for exactly 32 random bytes; raw text, placeholders, short strings, low-entropy values, empty files, and ordinary files with group/other permissions fail closed. Prefer the file form:
  ...
  ```
- Provision the backup evidence signing key as an approved secret source before any production/live preflight. Systemd hosts should install `/etc/credstore.encrypted/backup_evidence_hmac_key.cred` with `ops/server/credstore/install.sh` and set `BACKUP_EVIDENCE_HMAC_KEY_SECRET=backup_evidence_hmac_key`; the preflight units load it with `LoadCredentialEncrypted=backup_evidence_hmac_key:...`. Compose hosts should point `BACKUP_EVIDENCE_HMAC_KEY_FILE` at the Docker secret source file. A direct file source must be readable by the runtime service and mode `0600`; `root:trading 0640` backup-service files are not valid strict runtime sources.
...
- Install locked CPU/default Python dependencies with `TRADING_DEPENDENCY_PROFILE=cpu python -m pip install -r requirements.txt`.
...
- Replace existing local credentials with approved sources:
  ...
  - For file-backed deployments, write one secret per file with mode `0600` and owner `root:trading` or the service user. Docker `/run/secrets/*` mounts may use Docker's normal file mode, but they still must be readable and non-empty. Use names such as `dashboard_api_token`, `operator_api_token`, `timescale_password`, `redis_password`, `minio_root_user`, `minio_root_password`, `polygon_api_key`, `tradier_api_token`, `openai_api_key`, `alpaca_key_id`, and `alpaca_secret_key`.
