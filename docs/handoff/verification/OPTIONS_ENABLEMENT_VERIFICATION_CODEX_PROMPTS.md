...
> new feature work**.
>
> Run this as **one Codex task** (or split per-ID using the same protocol). Pair the original
> enablement file open beside this one — for each ID, re-read its *Definition of Done*, *Tests to add*,
> and *Validation commands*; this prompt tells you how to **prove** each was met.
